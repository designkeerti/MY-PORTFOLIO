...
export const BlockView = ({ b, accent }: { b: Block; accent: string }) => {
  switch (b.type) {
    ...
    case 'lead': return <div className="w-full max-w-[860px] mx-auto px-6"><p className="text-[20px] md:text-[25px] leading-[1.45] font-medium tracking-[-0.01em] text-white">{rich(b.text)}</p></div>;
    case 'h': return <div className="w-full max-w-[760px] mx-auto px-6 pt-6"><h3 className="text-[24px] md:text-[30px] font-bold tracking-[-0.02em] text-white">{b.text}</h3></div>;
    case 'note': return <div className="w-full max-w-[760px] mx-auto px-6"><div className="rounded-2xl border border-white/10 bg-white/[.04] p-6 text-[15.5px] leading-relaxed text-white/75"><span className="kicker block mb-2" style={{ color: accent }}>Note</span>{rich(b.text)}</div></div>;
    case 'quote': return (
      <div className="w-full max-w-[900px] mx-auto px-6">
        <Reveal><blockquote className="text-[22px] md:text-[32px] font-bold leading-[1.25] tracking-[-0.02em] text-white border-l-4 pl-6 md:pl-8" style={{ borderColor: accent }}>{b.text}{b.who && <footer className="kicker mt-4 text-white/50">{b.who}</footer>}</blockquote></Reveal>
      </div>
    );
    case 'list': return (
      <div className="w-full max-w-[760px] mx-auto px-6">
        <ul className="flex flex-col gap-3">
          {b.items.map((it, i) => (
            <li key={i} className="flex gap-4 items-start text-[17px] md:text-[18px] leading-relaxed text-white/85">
    ...
    case 'stats': return (
      <div className="w-full max-w-[1100px] mx-auto px-6">
        <Reveal className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {b.items.map(s => (
            <div key={s.l} className="rounded-2xl bg-white/[.05] p-6 md:p-7">
              <div className="font-black tracking-[-0.04em] leading-none" style={{ fontSize: 'clamp(34px, 4.6vw, 60px)', color: accent }}>{s.n}</div>
              <div className="text-[13.5px] md:text-[14.5px] text-white/65 mt-3 leading-snug">{s.l}</div>
            </div>
          ))}
        </Reveal>
      </div>
    ...
    case 'cards': return (
      <div className="w-full max-w-[1100px] mx-auto px-6">
        <div className={`grid gap-4 ${b.cols === 2 ? 'md:grid-cols-2' : 'md:grid-cols-3'}`}>
          {b.items.map((c, i) => (
            <Reveal key={i} className="rounded-2xl border border-white/[.08] bg-white/[.04] p-6 md:p-7">
              {c.kicker && <div className="kicker mb-3" style={{ color: accent }}>{c.kicker}</div>}
              <h4 className="text-[19px] md:text-[21px] font-bold tracking-[-0.015em] text-white mb-2">{c.title}</h4>
              <p className="text-[15px] md:text-[16px] leading-relaxed text-white/65">{rich(c.text)}</p>
            </Reveal>
          ))}
        </div>
      </div>
    ...
    case 'img': return (
      <figure className={`w-full ${widths[b.size ?? 'wide']} mx-auto px-6`}>
        <Reveal>
          ...
          {b.caption && <figcaption className="text-[14px] text-white/55 mt-3 max-w-[70ch]">{b.caption}</figcaption>}
        </Reveal>
      </figure>
    );
    case 'grid': {
              ...
              <div className={`grid gap-4 md:gap-6 ${cls} ${b.phone ? 'items-end' : ''}`}>
                {b.images.map((im, i) => (
                  <div key={i} className={b.phone ? 'flex flex-col items-center gap-3' : ''}>
                    ...
                    {im.caption && <div className={`text-[13.5px] mt-2 leading-snug ${b.bg ? 'text-white/70 text-center' : 'text-white/55'}`}>{im.caption}</div>}
                  </div>
                ))}
              </div>
            </div>
