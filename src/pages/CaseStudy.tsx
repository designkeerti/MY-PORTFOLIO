...
export const CaseStudy = ({ onDarkModeChange }: { onDarkModeChange: (d: boolean) => void }) => {
  ...
  useEffect(() => scrollYProgress.on('change', () => onDarkModeChange(false)), [scrollYProgress, onDarkModeChange]);
  useEffect(() => { onDarkModeChange(false); return () => onDarkModeChange(false); }, [onDarkModeChange, slug]);
  ...
  return (
    <article className="bg-white">
      <div ref={heroRef} className="bg-[#fbf9ff] text-[#1b1530] pt-24 md:pt-28 pb-16 md:pb-24 relative">
        <div className="absolute inset-x-0 top-0 h-[2px] bg-black/[.06]"><motion.div className="h-full" style={{ width: bar, background: meta.accent }} /></div>

        <div className="max-w-[1240px] mx-auto px-4 md:px-8">
          <div className="kicker text-[#928ba8] px-2 md:px-0 mb-4">{meta.company} · {meta.year}</div>
        ...
        <div className="max-w-[1000px] mx-auto px-6 pt-12 md:pt-16">
          ...
          <p className="text-[#5d5677] text-[17px] md:text-[20px] leading-relaxed max-w-[62ch] mt-6">{project.summary}</p>
          <dl className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-10 pt-8 border-t border-black/[.07]">
            {[['Company', meta.company], ['Role', meta.role], ['Expertise', project.expertise], ['Timeline', project.timeline]].map(([k, v]) => (
              <div key={k}><dt className="kicker text-[#928ba8] mb-2">{k}</dt><dd className="text-[15px] md:text-[16px] text-[#1b1530] leading-snug">{v}</dd></div>
            ))}
          </dl>
      ...
      {nextIsHere ? (
        <Link to={`/work/${next.slug}`} className="block bg-[#f3efff] text-[#1b1530] group">
          <NextCard label={next.title} accent={next.accent} sub={`${next.company} · ${next.year}`} />
        </Link>
      ) : (
        <a href={projects[(idx + 1) % projects.length].externalLink ?? '/'} target="_blank" rel="noreferrer" className="block bg-[#f3efff] text-[#1b1530] group">
          <NextCard label={next.title} accent={next.accent} sub={`${next.company} · ${next.year}`} />
        </a>
      )}
    </article>
...
const NextCard = ({ label, accent, sub }: { label: string; accent: string; sub: string }) => (
  <div className="max-w-[1000px] mx-auto px-6 py-20 md:py-28 flex flex-col md:flex-row md:items-end justify-between gap-8">
    <div>
      <div className="kicker text-[#928ba8] mb-4">Next project</div>
    ...
    <div className="kicker text-[#928ba8]">{sub}</div>
  </div>
);
