...
const VaultScene = ({ compact, tiny }: { compact: boolean; tiny: boolean }) => {
  const { phase } = useVaultTimeline(1150, true);
  return (
    <Bg color="#000">
      <WallHero state={heroStates[0]} enterDelay={1.25} compact={compact} tiny={tiny} />
      <Vault phase={phase} />
    </Bg>
  );
};

const StampScene = ({ compact }: { compact: boolean }) => {
  ...
  return (
    <Bg color="#06080b">
      <div className="cbs" style={{ position: 'absolute', inset: 0, padding: '4% 5% 58px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        {!compact && <div className="cbs-trust__intro cbs-trust__intro--noeyebrow" style={{ marginBottom: 14 }}>
        ...
        <Slide delay={0.2} from="bottom" distance={30}>
          <StampSheet items={compact ? brands.slice(0, 6) : brands} lit={lit} cols={compact ? 3 : 6} style={{ gap: compact ? 8 : 10 }} />
        </Slide>
      </div>
    </Bg>
  );
...
export const cbsScenes = (): Scene[] => [
  { id: 'vault', duration: 5200, caption: 'A first visit starts behind a steel door', render: ({ compact, w }) => <VaultScene compact={compact} tiny={w < 470} /> },
  {
    id: 'cards', duration: 3600, caption: 'Every product is a boarding pass: reference, route, perforation. Hover one.',
    render: ({ compact }) => (
      <Bg color="#06080b">
        <div className="cbs" style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', gap: '2.5%', padding: '5% 4% 0' }}>
          {sampleCards.slice(0, compact ? 2 : 4).map((c, i) => (
            <Slide key={c.code} delay={0.1 + i * 0.14} from="bottom" distance={90}
              style={{ width: compact ? '45%' : '23%', flex: '0 0 auto' }}>
              <BoardingCard {...c} />
            </Slide>
          ))}
        </div>
  ...
  },
  {
    ...
    render: ({ compact }) => (
      <Bg color="#06080b">
        <div className="cbs" style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', gap: '3%', padding: '5% 5% 0' }}>
          {stacks.slice(0, compact ? 1 : 2).map((s, i) => (
            <Slide key={s.num} delay={0.1 + i * 0.22} from="bottom" distance={110} rotate={i ? 2 : -2}
              style={{ width: compact ? '76%' : '44%', flex: '0 0 auto' }}>
              <StackTicket {...s} />
            </Slide>
          ))}
        </div>
  ...
  { id: 'stamps', duration: 3400, caption: 'Eleven brands on a sheet of stamps, each hiding its country', render: ({ compact }) => <StampScene compact={compact} /> },
  {
    id: 'live', duration: 3400, caption: 'Built on a live store taking real orders. No git, no staging, one change at a time.',
    render: ({ compact }) => (
      <Bg color="#06080b">
        <div className="absolute inset-0 flex items-center justify-center gap-[7%] px-[6%]" style={compact ? { flexDirection: 'column', alignItems: 'flex-start', gap: 18, paddingTop: '10%' } : undefined}>
          {[[81, ' ms', 'blocking long tasks on the home page, from 1,249'], [321, '', 'URLs audited at four widths, zero failures'], [76, ' MB', 'decoded image memory on a phone, from 144']].map(([n, s, l], i) => (
            <Slide key={String(l)} delay={0.15 + i * 0.2} from="bottom" distance={40} style={{ maxWidth: compact ? '100%' : '28%' }}>
